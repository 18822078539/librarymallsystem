...
<div class="layui-form-item" style="margin:15px;height:30px;">
    	<div style="height:40px;width:70px;float:left;line-height:40px;">任务标题:</div>
	...
	<form class="layui-form" id="typeForm">
		...
		<div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      <input type="text" name="taskTitle" id="taskTitle" required lay-verify="required" value="" class="layui-input" />
			    </div>			    
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">任务分数</label>
			<div class="layui-input-block">
				<input type="text" name="taskScore" id="taskScore" required lay-verify="required" value="" class="layui-input" />
			</div>
		</div>
	</form>
</div>
<div id="addData" style="width:300px;display:none;padding-top:15px;">
	<form class="layui-form" id="addTypeForm">
		<div class="layui-form-item">
			    <label class="layui-form-label">任务标题</label>
			    <div class="layui-input-block">
			      <input type="text" name="taskTitle" required lay-verify="required" value="" class="layui-input" />
			    </div>			    
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">任务分数</label>
			<div class="layui-input-block">
				<input type="text" name="taskScore" required lay-verify="required" value="" class="layui-input" />
			</div>
		</div>
	</form>
</div>
...
var form;
        layui.use(['layer', 'table','form'], function ()
        {
            ...
            var tableIns = table.render({
                elem: '#type'
                , id: 'type'
                , url: 'devoteTask/findTypeBySplitPage'
                ,width:1100
                , cols: [[
                     { checkbox: true, LAY_CHECKED: false } //其它参数在此省略
                     ,{ field: 'taskId', title: '任务编号', width: 100, align: 'center' }
                     , { field: 'taskTitle', title: '任务标题', width: 300, align: 'center'}
                    , { field: 'taskScore', title: '贡献分', width: 300, align: 'center'}
                    ,{ field:'taskState', title: '任务状态', width: 112, align: 'center',templet: function(d){
                          if(d.taskState==0){
                        	  return "<input type='checkbox' lay-filter='switchTest' name='"+d.taskId+"' lay-skin='switch' lay-text='禁用|正常'>";
                          }else{
                        	  return "<input type='checkbox' lay-filter='switchTest' name='"+d.taskId+"' lay-skin='switch' lay-text='禁用|正常' checked>";
                          }
                	 }
            ...
            $("#add").click(function(){
            	layer.open({
                    ...
                    yes: function(index, layero){
                    	$.post('devoteTask/addDevoteTask',$('#addTypeForm').serialize(),function(data){
                            if (data == 'success')
                            {
                                parent.layer.msg('添加任务信息成功！', { icon: 1, shade: 0.4, time: 1000 });
                                $("#search").click();
                            }
                            else
                            {
                                parent.layer.msg('添加任务信息失败！', { icon: 5, shade: 0.4, time: 1000 });
                            }
                            layer.close(index);
                    	}); 
                    }
            ...
            $("#delete").click(function ()
            {
                ...
                if (count > 0)
                {
                    parent.layer.confirm('确定要删除所选任务？', { icon: 3 }, function (index)
                    {
                        var data = checkStatus.data; //获取选中行的数据
                        var batchId = '';
                        for (var i = 0; i < data.length; i++)
                        {
                        	batchId += data[i].taskId + ",";
                        }
                        $.ajax({
                            url: 'devoteTask/batchDelete',
                            type: "post",
			...
			form.on('switch(switchTest)',function(data){
				...
				if(flag){
					$.ajax({
						type:"post",
						url:"devoteTask/changeTaskState",
						data:"typeId="+id+"&state="+1,
						success:function(data){
							if(data=="success"){
								layer.msg('禁用任务成功！',{icon:1,time:2000});
							}else{
								layer.msg('禁用任务失败！请重试！',{icon:5,time:2000});
							}
						}
					});
				}else{
					$.ajax({
						type:"post",
						url:"devoteTask/changeTaskState",
						data:"typeId="+id+"&state="+0,
						success:function(data){
							if(data=="success"){
								layer.msg("启用任务成功！",{icon:1,time:2000});
							}else{
								layer.msg("启用任务失败！请重试！",{icon:5,time:2000});
							}
						}
					});
				}
                ...
                { //编辑
                    layer.open({
                        ...
                        success: function (layero, index)
                        {
                            ...
                            $("#taskScore").val(data.taskScore);
                            form.render();
                            body.find('#hidValue').val(index); //将本层的窗口索引传给子页面层的hidValue中
                        },
                        btn:['修改','取消'],
                        yes: function(index, layero){
                        	$.post('devoteTask/updateDevoteTask',$('#typeForm').serialize(),function(data){
                                if (data == 'success')
                                {
                                    parent.layer.msg('修改任务信息成功！', { icon: 1, shade: 0.4, time: 1000 });
                                    $("#search").click();
                                    $("#handle_status").val('');
                                }
                                else
                                {
                                    parent.layer.msg('修改任务信息失败！', { icon: 5, shade: 0.4, time: 1000 });
                                }
                                layer.close(index);
                        	}); 
                        }
                	...
                	layer.confirm('是否删除该任务？', {
                		  ...
                		  ,btn1: function(index, layero){
                			  $.ajax({
                				   type: "POST",
                				   url: "devoteTask/deleteDevoteTask",
                				   data: "typeId="+data.taskId,
